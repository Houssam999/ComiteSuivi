...
It computes the fractal dimension for each segment, measures segment widths,
and saves visualizations of the processed images.
"""
...
# Definition of the class CompareNeighborsWithValue (algorithm 3)
...
def find_segments(skeleton, labeled_nodes, label_to_position):
    ...
    Each segment is associated with its start and end nodes.
    """
...
def process_frame(args):
    ...
    compute fractal dimensions, and generate visualizations.
    """
    ...
    # Create DataFrames for nodes and segments
    ...
    # Create a CSV for the fractal dimension of veins
    fractal_info = segments_df[['Segment_ID', 'Fractal_Dimension']]
    fractal_info.to_csv(os.path.join(output_dir, f'fractal_info_frame_{frame_idx + 1}.csv'), index=False)
...
def main():
    ...
    # Number of processes (adjust based on available resources)
    num_processes = 40
    ...
    print("Analysis completed. The CSV files and images have been saved for each frame.")
